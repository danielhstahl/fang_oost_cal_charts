...
Where \(\psi_l(u)=\lambda\left(e^{ui\mu_l-\frac{ u^2 \sigma_l^2}{2}}-1\right)+C\Gamma(-Y)\left((M-iu)^Y-M^Y+(G+iu)^Y-G^Y\right)\) is the log of the characteristic function (divided by \(t\)) of the combined Merton and CGMY process.
