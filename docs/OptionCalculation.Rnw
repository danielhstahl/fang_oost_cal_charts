...
\[x_t=\mathrm{log}\left(\frac{S_t}{S_0}\right)=\left(\alpha-\frac{\sigma^2}{2}\right)t+\sigma W^1_t+\sum_{j=1}^{N_t} y_j\]
Here \(dW^1_t dW^2_t=\rho dt\) and \(N_t\) is a Poisson process.  \(y_j\) is assumed to be Gaussian and independent of every other source of randomness. 
...
\[\phi_x(u)=e^{ui rt} g(-\psi(u))\]
Where \(g(z)=\mathbb{\hat{E}}[e^{-z \tau}]\) is the extended bond pricing formula for a CIR process.
...
The methodology for calibration uses the \href{http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.90.8837&rep=rep1&type=pdf}{Belomestny and Reiss (2006)} framework.  For more details and tests, see the \href{https://github.com/phillyfan1138/fang_oost_cal_charts/blob/master/docs/OptionCalibration.pdf}{Option Calibration} documentation.
